...
/*----------------------------------------------------------*/
/*-----------------------------------------DDL-----------------*/
DROP TRIGGER IF EXISTS upade_noaptea  ON DATABASE;  
GO
CREATE TRIGGER upade_noaptea
on DATABASE
FOR ALTER_TABLE
AS
SET NOCOUNT ON
...
SET @Finish_Time='06:00'

IF NOT DATEPART(hour, GETDATE()) between DATEPART(hour, @Start_Time) AND DATEPART(hour, @Finish_Time)
BEGIN
    RAISERROR ('Este interzisa modificarea in Baza pe timp de noapte', -- Message text.  
      11, -- Severity,  
      1);
    ROLLBACK;
END
...
/*--------------DDL-----------------nu pot face add, drop, la coloana, nimic---------------------------*/
DROP TRIGGER IF EXISTS nu_se_modifica ON DATABASE;  
GO
CREATE TRIGGER nu_se_modifica
on DATABASE
for ALTER_TABLE
AS
SET NOCOUNT ON
DECLARE @Telefon varchar(50)
SET @Telefon =EVENTDATA().value('(/EVENT_INSTANCE/AlterTableActionList/*/Columns/Name)[1]','nvarchar(max)')
IF @Telefon='Telefon'
BEGIN
    RAISERROR ('Coloana Telefon nu poate fi modificată.', -- Message text.  
      11, -- Severity,  
      1);
    ROLLBACK;
END
GO
/*----------------------DML------------------------------------*/
CREATE TRIGGER del_angajat ON Restaurante.Angajat AFTER DELETE
AS
	DECLARE @angajat_del varchar(20)
	SELECT @angajat_del = Nume FROM deleted
	IF @angajat_del = 'Alexandra%'
	BEGIN
        RAISERROR ('Angajatul Alexandra nu poate fi stearsa', 19,1)
        ROLLBACK TRANSACTION
    END
/*----------------------DML------------------------------------*/
GO
CREATE TRIGGER insertion ON Restaurante.Client  FOR INSERT AS
SET NOCOUNT ON
IF (SELECT COUNT(*) FROM inserted) !=
   (SELECT COUNT(*) FROM Restaurante.Client, inserted WHERE (Restaurante.Client.Telefon = inserted.Telefon))
    BEGIN
        RAISERROR ('The record can''t be added. Referential integrity rules require a related record in table ',44444,1,'.')
        ROLLBACK TRANSACTION
    END
/*----------------------------------------------------------*/
/*----------------------------------------------------------*/
IF EXISTS (SELECT TOP 1 * FROM master.dbo.sysdevices WHERE name='Backup_Lucr_An1')
BEGIN
  EXEC sp_dropdevice 'Backup_Lucr_An1' , 'delfile';
END;

EXEC sp_addumpdevice 'DISK', 'Backup_Lucr_An1','G:\proiect\a.bak'
GO 
BACKUP DATABASE PizzaRetea
TO Backup_Lucr_An1 WITH FORMAT,
NAME = N'Backup_Lucr_AnBackup'
GO


--2 Sa se scrie instructiunea unui 
--backup diferentiat al bazei de date universitatea. 
--Fișierul copiei de rezerva sa se numeasca exercitiul2.bak.

USE master;
BACKUP DATABASE PizzaRetea
TO DISK = 'G:\proiect\a2.bak'
WITH DIFFERENTIAL 
GO 
BACKUP LOG PizzaRetea TO DISK = 'G:\proiect\a2.bak'
GO

--4.Să se execute restaurarea consecutivă a tuturor copiilor de rezerva create. 
--Recuperarea trebuie să fie realizată într-o bază de date noua universitatea_lab11. 
--Fișierele bazei de date noi se afla în dosarul BD_lab11. Să se scrie instrucțiunile SQL respective.

IF EXISTS (SELECT * FROM master.sys.databases WHERE name='PizzaRetea')
DROP DATABASE PizzaRetea;
GO
RESTORE DATABASE PizzaRetea
    FROM PizzaReteaBackup
    WITH NORECOVERY,
      MOVE 'PizzaRetea' TO 'G:\data.mdf',
      MOVE 'PizzaRetea_log' TO 'G:\logs.ldf';


GO
RESTORE LOG universitatea_lab11
FROM DISK = 'G:\PizzaReteaBackup.bak'
WITH NORECOVERY
GO
